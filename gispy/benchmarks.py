...
class Wolfe:
    ...
    @staticmethod
    def wolfe_eval(input_file, prefix_names, variables, use_wolfe_vars=False, use_gispy_vars=False):
        df = pd.read_csv(input_file)
        df = GIS().score(df, variables, wolfe=use_wolfe_vars, gispy=use_gispy_vars)

        id_col = 'd_id'

        df_yes = df[df[id_col].apply(lambda x: x.split("_")[0].startswith(prefix_names['gist_yes']))]
        df_no = df[df[id_col].apply(lambda x: x.split("_")[0].startswith(prefix_names['gist_no']))]
    ...
    @staticmethod
    def run_benchmark(gispy_indices_file: str, prefix_names: dict, use_wolfe_vars=False, use_gispy_vars=True):
        """
        ...
        for vars_dict in vars_dicts:
            ...
            vars_name = GisPyData().generate_variables_dict_id(vars_dict)
            results = results.append({'vars_name': vars_name,
